...
| 稍后提醒 | 提醒横幅与托盘菜单都能「10 分钟 / 1 小时 / 明天」推迟，到点再提醒一次 |
| 快速添加 | 全局快捷键 `Ctrl+Alt+N`，一句话解析出时间、优先级、分类 |
...
- **稍后提醒**：提醒横幅上有「10 分钟 / 1 小时 / 明天」三个按钮；托盘右键菜单里也有同样的「稍后提醒」。推迟期间不再打扰，到点会以「稍后提醒」再提示一次。
- **快速添加**：按 `Ctrl+Alt+N`（或托盘菜单 / 工具栏闪电按钮）弹出输入框，一句话即可：
  - 例：`交周报 明天 10:00 #工作 !高`
  - 时间：`今天 / 明天 / 后天 / 周X / 12月25日 / 12-25`、`10:00`、`下午3点`、`2小时后`
  - 其他：`#分类名`、`!紧急 / !高 / !中 / !低`
  - 输入时下方实时显示解析结果，回车添加、`Esc` 取消；没写的部分按默认（明天 09:00、中优先级）处理
