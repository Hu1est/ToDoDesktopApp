...
- 任务到期时以**应用内弹窗 + 灵动岛**提醒
- 第一次点关闭会问一次「后台常驻 / 彻底退出」，选择会被记住、之后不再询问（也能在「设置 → 退出操作」里改）
...
| 退出操作可选 | 第一次关闭时询问一次，选择被记住后在设置里可随时切换 |
...
**只在第一次关闭窗口时询问一次**，选择会被记住，之后点关闭就直接按它执行：
...
- 第一次弹出的选择框里还有「取消」，表示「先不关」；此时还没做出选择，所以下次关闭仍会询问一次。
- 设置 → **退出操作** 只提供「后台常驻 / 彻底退出」两个选项，点选立即生效、以后不再询问；还没选过时两个选项都不高亮，并提示「第一次关闭窗口时会询问一次」。
...
- **首次启动说明**：新用户会看到 5 条带「示例」标记的示例任务和一段可关闭的提示卡片，卡片里说明示例任务不提醒、顶部各图标的作用、以及提醒到来时怎么用「稍后」；点「清空示例任务」可一键删掉这 5 条（你自己建的任务不受影响），点「知道了」后不再出现。
  - 示例任务**只在真正的首次启动生成一次**：之后不管你把任务删到 0 条还是删光示例任务，重新打开都不会再自动出现（不会出现「每次启动都变回初始状态」）。
